...
def get_assign_binary_iamge(image, threshold=160):
    ...
    # 转化到灰度图片
    image.save(r'..\VerifyCode-master\VerifyCode-master\img_data\new_' + str(int(time.time())) + str(random.randint(10, 99)) + '.png')
...
if __name__ == '__main__':
    file_path = '123.png'
    # file_path = 'GJX744.jpeg'
    img = cv2.imread(file_path)
    cv2.namedWindow('Image')
    cv2.imshow('Image', img)
    image = Image.open(file_path)
    new_img = ImageDispose(image).dispose(line=True, point=True, border=False)
    # cv2.namedWindow('NewImage')
    # cv2.imshow('NewImage', new_img)
    # cv2.imwrite('new_' + file_path, new_img)
    # tessdata_dir_config = '--tessdata-dir "C:\\Program Files (x86)\\Tesseract-OCR\\tessdata"'
    # print(pytesseract.image_to_string('new_' + file_path, config=tessdata_dir_config).replace(' ', ''))
